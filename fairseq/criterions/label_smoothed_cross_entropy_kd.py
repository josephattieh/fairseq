...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, teacher_model=None, reduce=True):
        ...
        if teacher_model is not None:
            ...
            kd_loss = self.compute_kd_loss(student_logits, teacher_logits, sample=sample)
    ...
    def compute_kd_loss(self, student_logits, teacher_logits, sample=None):
        teacher_probs = utils.softmax(teacher_logits, -1)
        teacher_lprobs = utils.log_softmax(teacher_logits, -1)
        student_lprobs = utils.log_softmax(student_logits, -1)
